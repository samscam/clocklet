...
struct ClockSummaryView: View {
    ...
    @State var viewModel: ClockSummaryViewModel
    @Environment(\.colorScheme) var colorScheme: ColorScheme
    
    var body: some View {
        
        ZStack{
            self.viewModel.image.resizable().aspectRatio(contentMode: .fit)
...
//            }
            Text(self.viewModel.title).font(.largeTitle).bold().shadow(color: colorScheme == .light ? .white : .black, radius: 2, x: 0, y: 0).padding(.top, 100)
