...
// Function prototypes:
void SolTrack(struct Time time, struct STLocation location, struct Position *position,  int useDegrees, int useNorthEqualsZero, int computeRefrEquatorial, int computeDistance);
...
void convertEquatorialToHorizontal(struct STLocation location, struct Position *position);
...
void SolTrack_RiseSet(struct Time time,  struct STLocation location, struct Position *position, struct RiseSet *riseSet, double sa0, int useDegrees, int useNorthEqualsZero);
